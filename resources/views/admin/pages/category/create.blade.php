        ...
        <form action="{{route('admin.category.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                ...
                <textarea type="text" name="description" id="description" placeholder="Full Name"
                    class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md">{{ old('title') }}</textarea>
                @error('description')
                <p class="text-red-600">* {{$message}}</p>
                @enderror
            </div>
