        ...
        <table class="w-full">
            ...
            <tbody class="border w-full">
                ...
                <tr class="flex justify-around w-full">
                    ...
                    <td>{{$item->title}}</td>
                    <td>
                        <img class="h-[50px] w-[50px] rounded-full" src="{{$item->image??'/images/default.png'}}" alt="">
                    </td>
                    <td class="flex gap-2 pt-2 pb-2">
                        <a href="{{route('admin.course.edit',$item->id)}}" class="border border-green-500 bg-green-300 text-green-800 px-2"><i class="ri-edit-line"></i></a>
                        <form id="deleteForm_{{$item->id}}" action="{{route('admin.course.delete',$item->id)}}" method="post">
                            @csrf
                            @method('delete')
